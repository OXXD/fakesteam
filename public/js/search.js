...
$(() => {
    ...
    $expander.on('click', function (e) {
        ...
        var $this = $(this);
        if (
            $this.prev().children(':gt(4)').is(':hidden')
        ) {
...
$(() => {
    ...
    var $searchTerm = $('.search_term'),
        kw = '',
        sortby = $selectContainer.children('a.trigger').data('sortby');
    sortby_copy = sortby;
    ...
    $tbc.on('click', function (e) {
        ...
        if (!$this.is('.checked')) {
            ...
            tags.push(tag);
            $termcontainer.append(`<div class="searchtag ellipsis">
                <span class="ellipsis">${$this.children().last().html()}</span>
        ...
        loadSearchResult(1, 15, kw, tags.toString(), sortby_copy);
    });
    ...
    function search2Obj() {
        ...
        if (location.search != '') {
            ...
            for (var str of arr) {
                // var key = str.split("=")[0]; var value = str.split("=")[1];
    ...
    if (location.search.indexOf('kw') > 0) {
        ...
        // console.log(kw);
        appendKw();
        loadSearchResult(1, 15, kw, tags.toString(), sortby);
    } else {
        loadSearchResult(1, 15, kw, tags.toString(), sortby);
    }
    ...
    function appendKw() {
        if (kw) {
            if ($termcontainer.children().hasClass('kw')) {
                $termcontainer.children('.kw').html(`<span class="ellipsis">${kw}</span><a href="" class="btn_close"></a>`);
            } else {
                $termcontainer.append(`<div class="searchtag kw ellipsis">
          <span class="ellipsis">${kw}</span>
          <a href="" class="btn_close"></a>
        </div>`);
            }
        } else {
    ...
    // 分页搜索功能
    function loadSearchResult(pno = 1,pageSize = 15,kw = '',tags = '',sortby = 0) {
        $.ajax({
            type: 'get',
            url: '/search',
            data: {
            ...
            success: function (response) {
                    ...
                    html += `<a href="app.html?appid=${item.appid}" class="search_result_row">
                      ...
                      <div class="col search_review_score">
                          <span class="search_review_summary ${
                              random3 == 1
                                  ? 'positive'
                                  : random3 == 2 ? 'mixed' : 'negative'
                          }"></span>
                      </div>
                      <div class="col discount_block sm">
                          <span class="discount_pct" style="display:${
                              Math.floor(Math.random() * 2) >= 1 ? 'none' : ''
                          }">-${Math.ceil(Math.random() * 7)}0%</span>
                      </div>
                ...
                page - 2 > 0 &&
                    (html += `<span class="pagelink">${page - 2}</span>`);
                page - 1 > 0 &&
                    (html += `<span class="pagelink">${page - 1}</span>`);
                ...
                <span class="end">${
                    page * pageSize > response.totalRecord
    ...
    // 排序
    $selectContainer.on('click', '.dropdown_menu a', function (e) {
        e.preventDefault();
        var $this = $(this),
            sortby = $this.data('sortby');
        sortby_copy = sortby;
        $selectContainer.children('a.trigger').html(`${$this.html()}<span class="down_arrow"></span>`).attr('data-sortby', sortby);
        loadSearchResult(1, 15, kw, tags.toString(), sortby);
    });
    ...
    $pages.on('click', 'span', function () {
        ...
        if (!$this.is('.disabled') && !$this.is('.active')) {
            if ($this.is('.prev')) {
                loadSearchResult(parseInt($pages.children('.active').html()) - 1, psize, kw, tags.toString(), sortby_copy);
            } else if ($this.is('.next')) {
                loadSearchResult(parseInt($pages.children('.active').html()) + 1, psize, kw, tags.toString(), sortby_copy);
            } else {
                loadSearchResult($this.html(), psize, kw, tags.toString(), sortby_copy);
            }
        }
    });
});

// #tagSuggest tag 搜索更多标签显示功能
$(() => {
